...
def get_xml_data_by_title(title):
    link = 'https://th.wikipedia.org/w/api.php?action=query&redirects&format=xml&rvprop=content&prop=extracts|revisions|pageprops|templates|categories&rvslots=main&titles=' + \
...
def get_first_paragraph(title):
    link = 'https://th.wikipedia.org/w/api.php?format=json&action=query&prop=extracts&exintro&explaintext&titles=' + \
        urllib.parse.quote(title)
    response = requests.get(link, timeout=30)  # 30s
...
def get_triple(word, level=2):
    ...
    for i in instances:
        triple_list.append([term_dict['label'], 'instance_of', i[1]])
    for s in subclasses:
        triple_list.append([term_dict['label'], 'subclass_of', s[1]])
    for p in parts:
        triple_list.append([term_dict['label'], 'part_of', p[1]])

    # get property data
    triple_list += get_extend_triple(triple_list,
                                     instances, subclasses, parts, level)
...
def get_text_not_by_section(text):
    # print('raw text: ', text)
    list_headers = re.findall(
        '<(h1|h2|h3|h4|h5|h6)>(.*?)</(h1|h2|h3|h4|h5|h6)>', text)
...
def extract_first_sentence(text):
    ...
    for w in text:
        ...
        if (w == '.'):
            if (len(first_sentence.split()) > 10 and count == 0):
                break
            ...
            if (len(qual_properties) == 0):
                ...
                try:
                    for x in claim.find('qualifiers'):
                        ...
                        q = remove_emojis(x.attrib['id'])
                        # qualifier data type
                        qt = remove_emojis(x[0].attrib['datatype'])
                        subr = [q, qt]
...
def get_wikidata_item_by_name(item_name, file_name, depth):
    ...
    write_wikidata_to_csv_file(
        file_name, wikidata_id, label, description, alias, claims)
...
def get_hypernyms(values, results, level):
    ...
    if (level == 0 or len(values) == 0):
        results += values
        results = [list(x) for x in set(tuple(x)
                                        for x in results)]  # keep unique values
        return results
...
def map_page_list_to_wikidata(input_file, output_file):
    ...
    for p in page_list:
        try:
            ...
            write_wikidata_to_csv_file2(
                output_file, wikidata_id, p, description, alias_string, country_string)
        except Exception as e:
            # print(e)
            pass
...
def scan_people_from_list(file_name, output_file):
    ...
    for p in page_list:
        ...
        if (wikidata_id == '' or wikidata_id == None):
            continue
...
def update_to_blaze_graph(file_name, IP):
    ...
    for p in page_list:
        print(p[0])
        command = "curl -X POST http://" + IP + \
            "/blazegraph/sparql --data-urlencode 'update=DROP ALL; LOAD <https://www.wikidata.org/entity/" + \
            p[0] + ".ttl>;'"
        print(command)
        os.system(command)
